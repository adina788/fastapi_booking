...
def create_refresh_token(data: dict):
    return create_access_token(data=data, expire_delta=timedelta(days=REFRESH_EXPIRE_TOKEN))
...
async def register(user_data: RegisterSchema, db: Session = Depends(get_db)):
    ...
    if user_db:
        return HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Mynday adam registration bolgon')
